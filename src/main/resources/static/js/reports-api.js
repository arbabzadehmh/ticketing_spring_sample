...
function loadReports(page = 0) {
    ...
    if (firstLoad) {
        ...
        let url = `/reports?page=${encodeURIComponent(page)}&size=${encodeURIComponent(size)}&fragment=true`;
    ...
    let apiUrl = `/rest/reports?page=${encodeURIComponent(page)}&size=${encodeURIComponent(size)}`;
...
function renderReportsTable(reports) {
    ...
    reports.forEach(r => {
        ...
        row.innerHTML = `
            <td>${formatDateTime(r.fromDate)}</td>
            <td>${formatDateTime(r.toDate)}</td>
            <td>${r.ticketIds ? r.ticketIds.length : 0}</td>
            <td>${formatDateTime(r.createdAt)}</td>
            <td>
                <button class="btn btn-sm btn-info btn-open" data-id="${r.id}">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (pageLink) {
            ...
            if (!isNaN(page)) loadReports(page);
            return;
        }
        ...
        if (btnView) {
            ...
            if (id) window.location.href = `/reports/low-score/${id}`;
        }
    });
